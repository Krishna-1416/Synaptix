...
import {
  Activity, ArrowLeft, ArrowUpRight, BarChart3, Bell, Check, ChevronRight, CircleHelp,
  ClipboardCheck, FileText, History, ImagePlus, LayoutDashboard, LoaderCircle,
  LockKeyhole, LogOut, Mail, Menu, Moon, Search, Settings, ShieldCheck,
  SlidersHorizontal, Sun, UploadCloud, UserRound, X, XCircle
} from 'lucide-react'
...
function Scan({ onComplete, onCancel }) {
  ...
  const [error, setError] = useState('')
  async function submit(event) { event.preventDefault(); if (!file) return setError('Choose a product label image first.'); setSubmitting(true); setError(''); try { onComplete(await api.inspect({ file, productName, category })) } catch (caught) { setError(caught.message) } finally { setSubmitting(false) } }

...
function normalizeStats(stats = {}) {
  return {
    total:
      ...
      stats.inspections_count ??
      demoStats.total_inspections,
    pass:
      stats.compliant ??
      stats.passed ??
      stats.pass_count ??
    ...
    <div className="entry-screen splash-screen">
      ...
      <div className="splash-content">
        ...
        <div className="splash-wordmark">
          synaptix<span>field intelligence</span>
        </div>
  ...
  return (
    <header className="entry-header">
      <div className="entry-brand">
        <span className="entry-brand-mark">
          <ShieldCheck size={17} />
        </span>
...
function Welcome({ onNext, theme, onToggleTheme }) {
  return (
    <div className="entry-screen welcome-screen">
      <EntryHeader theme={theme} onToggleTheme={onToggleTheme} />
      <div className="welcome-content">
        <div className="welcome-copy">
          <div className="entry-eyebrow">
            <span /> INSPECTION INTELLIGENCE / 01
          </div>
          <h1>
            Clarity for every
            <br />
          ...
          <p>
            Synaptix turns a package photograph into a clear, defensible
            compliance decision for the officers who keep commerce honest.
          </p>
          ...
          <div className="welcome-meta">
            <span>
              <ShieldCheck size={15} /> Rule 6 aware
            </span>
            <span>
              <Activity size={15} /> OCR + vision pipeline
            </span>
          </div>
        </div>
        <div className="welcome-art">
          ...
          <div className="art-note note-one">
            ...
            <small>Every declaration, found.</small>
          </div>
          <div className="art-note note-two">
            <span>02</span>
            <strong>Rule validation</strong>
            <small>Every decision, traceable.</small>
          </div>
        </div>
      </div>
      <div className="entry-footer">
        <span>Built for enforcement teams</span>
        <span>
          Scroll to inspect <ChevronRight size={14} />
        </span>
      </div>
    </div>
  );
      ...
      <div className="portal-layout">
        <div className="portal-content">
          ...
          <p className="portal-intro">
            Open the Synaptix workspace for your inspection work.
          </p>
          <div className="portal-grid">
            <button className="portal-card selected" onClick={onSelect}>
              ...
              <div className="portal-card-copy">
                ...
                <p>
                  Scan labels, review declarations, and issue compliance
                  certificates.
                </p>
              </div>
          ...
          <div className="portal-help">
            <CircleHelp size={16} />
            <span>
              Your account will open the inspection workspace automatically.
            </span>
          </div>
        </div>
        <PortalVisual />
...
function PortalVisual() {
  return (
    <div className="portal-visual" aria-hidden="true">
      ...
      <div className="visual-chip chip-ocr">
        <Activity size={14} />
        <span>
          OCR found
          <br />
          <strong>06 fields</strong>
        </span>
      </div>
      <div className="visual-chip chip-pass">
        <Check size={14} />
        <span>
          Rule check
          <br />
    ...
    try {
      const result =
        mode === "login"
          ...
          : await api.signup({
              ...form,
              role: role === "user" ? "inspector" : "admin",
            });
      if (result.access_token)
        window.localStorage.setItem(
          "synaptix_access_token",
          result.access_token,
        );
      onAuthenticated({
        ...(result.user || {}),
        full_name:
          ...
          form.fullName ||
          (role === "user" ? "Synaptix User" : "Synaptix Admin"),
        role,
      });
    } catch (caught) {
      ...
      <div className="auth-layout">
          ...
          <h1>
            {mode === "login" ? (
              ...
              <>
                Make every
                <br />
                <em>decision count.</em>
              </>
            )}
          </h1>
          <p>
            {mode === "login"
              ? "Choose User or Admin access, then sign in to continue."
              : "Create an account with the access level your work requires."}
          </p>
        ...
        <div className="auth-card">
          ...
          <div className="auth-card-heading">
            ...
            <div>
              <h2>{role === "user" ? "User workspace" : "Admin workspace"}</h2>
              <p>
                {mode === "login"
          ...
          <form onSubmit={submit}>
              ...
              <label className="auth-field">
                ...
                <div>
                  ...
                  <input
                    ...
                    placeholder={
                      role === "user" ? "Riya Kapoor" : "Arjun Mehta"
                    }
                  />
                </div>
              </label>
            ...
            <label className="auth-field">
              ...
              <div>
                ...
                <input
                  ...
                  placeholder={
                    role === "user"
                      ? "user@department.gov.in"
                      : "admin@department.gov.in"
                  }
                />
              </div>
            </label>
            ...
            <button className="button auth-submit" disabled={busy}>
              {busy ? (
                <>
                  <LoaderCircle size={17} className="spinner" /> Connecting...
                </>
              ) : (
                <>
                  {mode === "login"
          ...
          <div className="auth-foot">
            {mode === "login" ? (
              <>
                Forgot your password? <button>Contact support</button>
              </>
            ) : (
              <>
...
function App() {
  const [theme, setTheme] = useState(
    () => window.localStorage.getItem("synaptix_theme") || "light",
  );
  const [authenticated, setAuthenticated] = useState(
    () => window.localStorage.getItem("synaptix_authenticated") === "true",
  );
  const [user, setUser] = useState(() =>
    JSON.parse(
      window.localStorage.getItem("synaptix_user") ||
        '{"full_name":"Riya Kapoor","role":"user"}',
    ),
  );
  ...
  useEffect(() => {
    ...
    Promise.all([api.getDashboardStats(), api.getInspections({ limit: 10 })])
      ...
      .catch(() => {
        if (mounted)
          setNotice(
            "API offline: showing sample inspection data. Set VITE_API_BASE_URL when connecting a deployed backend.",
          );
      })
  ...
  const selectedInspection = useMemo(
    () => inspections.find((item) => item.inspection_id === selectedId),
  ...
  function handleInspectionComplete(result) {
    setInspections((current) => [
      result,
      ...current.filter((item) => item.inspection_id !== result.inspection_id),
    ]);
      ...
      <aside className="sidebar">
        ...
        <div className="sidebar-footer">
          ...
          <div className="user-chip">
            ...
            <div>
              <strong>{user.full_name || "Riya Kapoor"}</strong>
              <span>
      ...
      <main className="main-content">
        <header className="topbar">
          ...
          <div className="topbar-actions">
            <span className={`connection-dot ${isDemo ? "offline" : ""}`}>
              <Activity size={14} /> {isDemo ? "Demo mode" : "API connected"}
            </span>
        ...
        {activeView === "analytics" && (
          <AnalyticsView stats={stats} inspections={inspections} />
        )}
        {activeView === "configuration" && (
          <ConfigurationView theme={theme} onToggleTheme={toggleTheme} />
        )}
      </main>
    </div>
  );
  ...
  return (
    <div className="page">
      <PageIntro
        ...
        description="Your compliance desk at a glance."
        action={
          <button className="button primary" onClick={() => onNavigate("scan")}>
            <ImagePlus size={17} /> Start inspection
          </button>
        }
      />
      ...
      <div className="content-grid">
        ...
        <section className="panel distribution-panel">
          <div className="panel-heading">
            <div>
              <div className="eyebrow">Compliance pulse</div>
          ...
          <div className="donut-wrap">
            <div
              className="donut"
              style={{
                "--pass": `${stats.total ? (stats.pass / stats.total) * 100 : 0}%`,
                "--fail": `${stats.total ? (stats.fail / stats.total) * 100 : 0}%`,
              }}
            >
              <div>
                <strong>{Number(stats.rate).toFixed(0)}%</strong>
                <span>compliant</span>
              </div>
            </div>
          </div>
          <div className="legend">
            <Legend color="green" label="Compliant" value={stats.pass} />
            <Legend color="red" label="Non-compliant" value={stats.fail} />
            <Legend color="yellow" label="Needs review" value={stats.review} />
          </div>
        </section>
      </div>
      <section className="insight-strip">
        ...
        <div>
          <strong>Rule 6 monitoring is active</strong>
          <span>
            Synaptix is checking mandatory declarations across every uploaded
            package label.
          </span>
        </div>
...
function AnalyticsView({ stats, inspections }) {
  ...
  const categoryRows = Object.entries(categories).sort(
          ...
          <div className="category-list">
            {(categoryRows.length
              ...
              <div className="category-row" key={category}>
                <div>
                  <span>{category}</span>
                  <small>{count} inspections</small>
                </div>
                <div className="category-track">
                  <i
                    style={{
                      width: `${Math.max(18, (count / Math.max(stats.total, 1)) * 100)}%`,
                    }}
                  />
                </div>
                <strong>
                  {Math.round((count / Math.max(inspections.length, 1)) * 100)}%
                </strong>
              </div>
            ))}
          </div>
    ...
    <div className="page">
      <PageIntro
        ...
        action={
          ...
          >
            <Check size={17} /> {saved ? "Saved" : "Save changes"}
          </button>
        }
      />
      <div className="configuration-grid">
        <section className="panel settings-panel">
          ...
          <div className="setting-row">
            <div>
              ...
              <span>Applied to new label inspections.</span>
            </div>
            <select
          ...
          <div className="setting-row">
            <div>
              <strong>OCR confidence threshold</strong>
              <span>Flag extracted values below this level.</span>
            </div>
            <select
          ...
          <div className="setting-row">
            <div>
              ...
              <span>Choose the appearance for this device.</span>
            </div>
        ...
        <section className="panel settings-panel">
          ...
          <ToggleRow
            ...
            detail="Notify me when an inspection needs attention."
            checked={settings.alerts}
            onChange={(value) => update("alerts", value)}
          />
          <ToggleRow
            label="Generate report after inspection"
            detail="Prepare the PDF certificate when processing finishes."
            checked={settings.autoReport}
            onChange={(value) => update("autoReport", value)}
          />
          <div className="configuration-status">
            <ShieldCheck size={16} />
            <span>Configuration is stored locally for this workspace.</span>
          </div>
        </section>
      </div>
    </div>
        ...
        <tbody>
          {inspections.map((item) => (
            <tr
              ...
              onClick={() => onOpen(item.inspection_id)}
            >
              <td>
                <strong>{item.inspection_id}</strong>
              </td>
              <td>
...
function Scan({ onComplete, onCancel }) {
  ...
  async function submit(event) {
    event.preventDefault();
    if (!file) return setError("Choose a product label image first.");
    setSubmitting(true);
    setError("");
    try {
      onComplete(await api.inspect({ file, productName, category }));
    } catch (caught) {
  ...
  function acceptFile(nextFile) {
    if (nextFile && nextFile.type.startsWith("image/")) {
      setFile(nextFile);
      setError("");
    } else setError("Please choose a JPG, PNG, or WEBP image.");
  }
  return (
    <div className="page scan-page">
      <PageIntro
        ...
        description="Upload a clear package image and let the pipeline check every declaration."
        action={
      ...
      <form className="scan-layout" onSubmit={submit}>
        <div className="scan-main">
          ...
          >
            ...
            {file ? (
              ...
              <label htmlFor="label-image">
                ...
                <span>
                  or <u>browse files</u> · JPG, PNG or WEBP up to 10 MB
                </span>
              </label>
            )}
          </div>
          <div className="scan-note">
            <ShieldCheck size={17} />
            <span>
              The image is processed by the Synaptix inspection pipeline. No
              source images are sent anywhere except your configured backend.
            </span>
          </div>
        </div>
        <aside className="scan-sidebar">
          <div className="panel form-panel">
            ...
            <label>
              Product name <span>Optional</span>
              <input
                value={productName}
            ...
            <label>
              ...
              >
                <option>Packaged Commodity</option>
                <option>Food & beverage</option>
                <option>Personal care</option>
                <option>Household</option>
                <option>Other</option>
            ...
            <button className="button primary wide" disabled={submitting}>
              {submitting ? (
                <>
                  <LoaderCircle className="spinner" size={17} /> Analysing
                  label...
                </>
              ) : (
                <>
                  <ClipboardCheck size={17} /> Run inspection
                </>
              )}
            </button>
          </div>
          <div className="pipeline-list">
            ...
            <PipelineStep
              number="01"
              title="Extract"
              text="OCR finds mandatory declarations."
            />
            <PipelineStep
              number="02"
              title="Check"
              text="Visual rules assess readability."
            />
            <PipelineStep
              number="03"
              title="Decide"
              text="Rule 6 produces the result."
            />
          </div>
        </aside>
      </form>
...
function HistoryView({ inspections, onOpen, onNavigate }) {
  const [search, setSearch] = useState("");
  const [filter, setFilter] = useState("ALL");
  const filtered = inspections.filter(
    (item) =>
      (filter === "ALL" || item.compliance?.status === filter) &&
      `${item.inspection_id} ${item.product?.name || ""}`
        .toLowerCase()
  ...
  return (
    <div className="page">
      <PageIntro
        ...
        title="History, with context."
        description="Search every label that has moved through the compliance pipeline."
        action={
          <button className="button primary" onClick={() => onNavigate("scan")}>
            <ImagePlus size={17} /> New inspection
          </button>
        }
      />
        ...
        <div className="filter-bar">
          ...
          <div className="filter-tabs">
            {["ALL", "PASS", "FAIL", "REVIEW"].map((value) => (
              ...
              >
                {value === "ALL" ? "All results" : statusMeta[value].label}
              </button>
            ))}
          </div>
        </div>
...
function Detail({ inspection, onBack }) {
  ...
  useEffect(() => {
    ...
    api
      .getInspection(inspection.inspection_id)
      .then(setFullInspection)
      .catch(() => {});
  }, [inspection]);
  if (!fullInspection)
    return (
    ...
    <div className="page">
      ...
      <div className="detail-grid">
        <section className="panel result-panel">
          ...
          {visual.overlay_image && (
            <div className="overlay-preview" style={{ marginTop: 24 }}>
              <div className="eyebrow">Detected text regions</div>
              <img
                src={visual.overlay_image}
                alt="Detected text regions highlighted on the label"
        ...
        <section className="panel declarations-panel">
          ...
          <div className="ocr-count">
            <Activity size={15} /> {ocr.texts?.length || 0} text regions
            detected by OCR
          </div>
        </section>
      </div>
